...
// Bakgrunnsbilder for øyer - LAG 1: Bakgrunnen
...
// Øy-bilder - LAG 2: Øyen (erstatter ringen)
const islandImages = {
...
// Fiende-bilder - LAG 3: Fienden (på toppen av øyen)
const enemyImages = {
...
// Boss-bilder - LAG 3: Boss-fienden
const bossImages = {
...
// ======================================================
// 3-LAGS SYSTEM: BAKGRUNN → ØY → FIENDE
// ======================================================
...
function spawnEnemy() {
    ...
    console.log(`Spawning enemy: Biome=${biome.name}, Level=${gameState.level}, Enemy=${gameState.enemyNumber}`);
    console.log(`Background: ${biome.bgImage}`);
    console.log(`Island: ${biome.islandImage}`);
    
    // LAG 1: Sett bakgrunnen (erstatter den lilla bakgrunnen)
    const mainBg = document.querySelector('.game-container');
    if (mainBg) {
        ...
        bgImg.onload = function() {
            console.log(`Background loaded: ${biome.bgImage}`);
        };
        bgImg.onerror = function() {
            console.error(`Failed to load background: ${biome.bgImage}`);
            mainBg.style.background = `linear-gradient(135deg, ${biome.color} 0%, ${darkenColor(biome.color, 20)} 100%)`;
        };
    ...
    if (islandContainer) {
        ...
        islandContainer.appendChild(enemyContainer);
        ...
        enemyImg.onload = function() {
            console.log(`Enemy image loaded: ${enemyImg.src}`);
        };
        enemyImg.onerror = function() {
            console.error(`Failed to load enemy: ${enemyImg.src}`);
            this.style.display = 'none';
...
function updateResources() {
    ...
    if (coinIcon) coinIcon.innerHTML = `<img src="${uiIcons.coin}" style="width:24px;height:24px;">`;
    if (gemIcon) gemIcon.innerHTML = `<img src="${uiIcons.gem}" style="width:24px;height:24px;">`;
    if (starIcon) starIcon.innerHTML = `<img src="${uiIcons.star}" style="width:24px;height:24px;">`;
}
